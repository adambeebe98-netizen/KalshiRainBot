#!/usr/bin/env bash
# Adds the web dashboard to an EXISTING kalshi_weather_bot install.
# Run this on your droplet, in the same directory setup you already have
# (it assumes ~/kalshi_weather_bot already exists from deploy.sh).
set -e
...
cat > web_ui/requirements.txt << 'REQEOF'
flask>=3.0
REQEOF

echo "Installing Flask into the existing virtual environment..."
source venv/bin/activate
pip install -r web_ui/requirements.txt -q

# Make sure a .env exists at all (in case setup was interrupted earlier)
touch .env

# Add a dashboard password if one isn't already set, without touching
# anything else already in .env.
if ! grep -q "^WEB_UI_PASSWORD=" .env 2>/dev/null; then
    NEW_PASS=$(python3 -c "import secrets; print(secrets.token_urlsafe(9))")
    echo "WEB_UI_PASSWORD=$NEW_PASS" >> .env
else
    NEW_PASS=$(grep "^WEB_UI_PASSWORD=" .env | cut -d= -f2)
fi

echo "Setting up the dashboard as a background service..."
sudo tee /etc/systemd/system/kalshi-bot-ui.service > /dev/null << SERVICEEOF
[Unit]
Description=Kalshi Weather Bot Web Dashboard
After=network-online.target

[Service]
Type=simple
User=$USER
WorkingDirectory=$APP_DIR/web_ui
EnvironmentFile=$APP_DIR/.env
ExecStart=$APP_DIR/venv/bin/python $APP_DIR/web_ui/app.py
Restart=on-failure
RestartSec=15
...
sudo systemctl enable kalshi-bot-ui
sudo systemctl restart kalshi-bot-ui

# Open the port if a firewall is active (harmless if ufw isn't installed/enabled).
if command -v ufw >/dev/null 2>&1; then
    sudo ufw allow 8080/tcp >/dev/null 2>&1 || true
fi

DROPLET_IP=$(curl -s -4 ifconfig.me || echo "YOUR_DROPLET_IP")
...
echo "    $NEW_PASS"
echo ""
echo "  (Also saved in $APP_DIR/.env as WEB_UI_PASSWORD if you lose it.)"
echo "=================================================="
echo ""
